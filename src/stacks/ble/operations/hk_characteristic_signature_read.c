#include "hk_characteristic_signature_read.h"
...
void hk_characteristic_signature_read_response(const ble_uuid128_t *characteristic_uuid, hk_session_t *session, hk_mem* response)
{
    ...
    tlv_data = hk_tlv_add_uint16(tlv_data, HK_TLV_CHARACTERISTIC_PROPERTIES, 
        hk_characteristics_properties_get_prop(session->characteristic_type));
