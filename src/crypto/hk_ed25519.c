...
esp_err_t hk_ed25519_update_from_random(hk_ed25519_key_t *key)
{
    ...
    int ret = 0;
    HK_CRYPTO_RUN_AND_CHECK(ret, wc_ed25519_init,k);
    HK_CRYPTO_RUN_AND_CHECK(ret, wc_InitRng,&rng);
    HK_CRYPTO_RUN_AND_CHECK(ret, wc_ed25519_make_key,&rng, ED25519_KEY_SIZE, k);
...
esp_err_t hk_ed25519_update_from_random_from_public_key(hk_ed25519_key_t *key, hk_mem *public_key)
{
    ...
    int ret = 0;
    HK_CRYPTO_RUN_AND_CHECK(ret, wc_ed25519_init, k);
    HK_CRYPTO_RUN_AND_CHECK(ret, wc_ed25519_import_public,(byte *)public_key->ptr, public_key->size, k);
