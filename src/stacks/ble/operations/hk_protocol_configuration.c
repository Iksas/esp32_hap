...
esp_err_t hk_protocol_configuration_response(const ble_uuid128_t *chr_uuid, hk_session_t *session)
{
    ...
    if (hk_tlv_get_tlv_by_type(tlv_data, 0x01) != NULL)
    {
        ...
        hk_pair_verify_keys_t *session_keys = hk_session_security_keys_get();

        // generate broadcast_key
        hk_mem *broadcast_key = hk_mem_create();
        hk_hkdf_with_external_salt(HK_HKDF_BROADCAST_ENCRYPTION_KEY, accessory_public_key, session_keys->shared_secret, broadcast_key);
